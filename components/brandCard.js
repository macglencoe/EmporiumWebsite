...
export default function ({ name, href, logoSrc, category, description, ...props }) {
    return (
        ...
        >
            <PolyWrapper href={href} {...props}>
                {/* Card Container */}
                <div className="relative h-full transition-all duration-300 group-hover:-translate-y-1 bg-primary2 bg-gradient-to-br from-primary1/10 via-secondary1/20 to-primary1/10"
                    style={{
...
function PolyWrapper({ children, ...props }) {
    if (props.href) {
        return (
            <Link href={props.href}>
                <a className="block h-full relative">
                    {children}
                </a>
            </Link>
        )
...
function BrandCardInner({ children, ...props }) {
